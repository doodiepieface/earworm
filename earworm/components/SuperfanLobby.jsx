...
export default function SuperfanLobby({
  claims, // [{ playerId, name, artist }] — no song counts; nothing is pulled yet
  meId,
  myClaim,
  onClaim,
  depth,
  ...
  return (
    <>
      <section className="section">
        ...
        {/* Claiming is instant — no songs are pulled here. Everyone's catalog is
            resolved once the host starts, so it's built from the depth setting
            the game actually begins with rather than whatever was selected the
            moment they happened to claim. */}
        <form className="artist-form" onSubmit={submit}>
          <input
            type="text"
            value={draft}
            placeholder={myClaim ? "Change your artist…" : "Who are you a superfan of?"}
            autoComplete="off"
            aria-label="Your artist"
            onChange={(e) => setDraft(e.target.value)}
          />
          <button type="submit" className="btn btn-primary" disabled={!draft.trim()}>
            {myClaim ? "Change" : "Claim"}
          </button>
        </form>
        {myClaim && (
          <p className="dim">
            You&rsquo;re repping <strong>{myClaim.artist}</strong>. Songs load when the
            host starts.
          </p>
        )}
        ...
        <ul className="claims">
          {claims.map((c) => (
            <li key={c.playerId} className={c.playerId === meId ? "me" : ""}>
              ...
              <span className="claim-state mono">{c.artist ? "ready" : "—"}</span>
            </li>
          ))}
