...
- **Claiming an artist is instant; catalogs are pulled at start.** Resolving at claim time froze each pool at whatever depth was set in that moment, so changing the depth afterwards did nothing for anyone who had already claimed. `start` now begins a **loading phase** instead of the game: every client resolves its own artist at the depth carried in that broadcast, reports progress via `loading { playerId, artist, count, done }`, and the host builds the round list once everyone is done. Progress is broadcast so the wait is shared rather than five private spinners.
- **The smallest pool clamps the mastery rounds** (`maxMastery` into `roomHost.start`). Pool sizes are only knowable after resolution, which is the other reason it happens at start — a thin catalog now shortens the game instead of wrapping the shuffle bag and repeating songs.
...
- **The finale is drawn *after* mastery, not at kickoff.** `roomHost.buildList` lays down `{ kind: "pending" }` placeholders (so `totalRounds` is right from the first broadcast) and `fillFinale()` replaces them the first time `directiveFor` reaches one. It has to be late: `finaleQuotas` hands the **spare rounds to whoever is losing** — 4 finale rounds across 3 artists gives last place two — and at kickoff everyone is on zero. Worst-first is `sortStandings` reversed, with no id tiebreak, so genuinely level players get a coin flip from the caller's shuffle rather than always the same name. Filling is keyed on the placeholders existing, which is what stops a host reconnect (`currentDirective()`) from reshuffling a finale already in progress.
